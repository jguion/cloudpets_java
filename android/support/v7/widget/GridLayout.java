...
public class GridLayout extends ViewGroup {
    ...
    final class Axis {
        ...
        private String arcsToString(List<Arc> arcs) {
            ...
            for (Arc arc : arcs) {
                ...
                if (src < dst) {
                    str = var + dst + Constants.FILENAME_SEQUENCE_SEPARATOR + var + src + ">=" + value;
                } else {
                    str = var + src + Constants.FILENAME_SEQUENCE_SEPARATOR + var + dst + "<=" + (-value);
                }
                result.append(str);
            }
    ...
    static Alignment getAlignment(int gravity, boolean horizontal) {
        switch ((gravity & (horizontal ? 7 : ParseException.INVALID_CHANNEL_NAME)) >> (horizontal ? 0 : 4)) {
            case 1:
                return CENTER;
            case 3:
