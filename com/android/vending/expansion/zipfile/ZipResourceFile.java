    ...
    public ZipEntryRO[] getEntriesAt(String path) {
        ...
        for (ZipEntryRO ze : values) {
            if (ze.mFileName.startsWith(path)) {
                int subpathSeparatorIndex = ze.mFileName.indexOf(47, length);
                if ((-1 == subpathSeparatorIndex && ze.mFileName.length() != length) || subpathSeparatorIndex == ze.mFileName.length() - 1) {
                    zev.add(ze);
                }
            }
        }
