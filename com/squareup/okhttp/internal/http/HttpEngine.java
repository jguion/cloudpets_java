...
public final class HttpEngine {
    ...
    private Request networkRequest(Request request) throws IOException {
        ...
        if ((this.connection == null || this.connection.getProtocol() != Protocol.HTTP_1_0) && request.header(HttpHeaders.CONNECTION) == null) {
                ...
                if (this.forWebSocket) {
                    ...
                    if (this.sentRequestMillis == -1) {
                        if (OkHeaders.contentLength(this.networkRequest) == -1 && (this.requestBodyOut instanceof RetryableSink)) {
                            this.networkRequest = this.networkRequest.newBuilder().header(HttpHeaders.CONTENT_LENGTH, Long.toString(((RetryableSink) this.requestBodyOut).contentLength())).build();
                        }
                        this.transport.writeRequestHeaders(this.networkRequest);
                    }
    ...
    private static boolean validate(Response cached, Response network) {
        ...
        if (lastModified != null) {
            Date networkLastModified = network.headers().getDate(HttpHeaders.LAST_MODIFIED);
    ...
    private static Headers combine(Headers cachedHeaders, Headers networkHeaders) throws IOException {
        ...
        for (i = 0; i < size; i++) {
            ...
            if (!HttpHeaders.CONTENT_LENGTH.equalsIgnoreCase(fieldName) && OkHeaders.isEndToEnd(fieldName)) {
    ...
    private static Address createAddress(OkHttpClient client, Request request) {
        ...
        return new Address(request.httpUrl().host(), request.httpUrl().port(), client.getSocketFactory(), sslSocketFactory, hostnameVerifier, certificatePinner, client.getAuthenticator(), client.getProxy(), client.getProtocols(), client.getConnectionSpecs(), client.getProxySelector());
    }
}
