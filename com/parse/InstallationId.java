...
class InstallationId {
    ...
    public String get() {
        synchronized (this.lock) {
            if (this.installationId == null) {
                try {
                    this.installationId = ParseFileUtils.readFileToString(this.file, Key.STRING_CHARSET_NAME);
    ...
    private void setInternal(String newInstallationId) {
        synchronized (this.lock) {
            try {
                ParseFileUtils.writeStringToFile(this.file, newInstallationId, Key.STRING_CHARSET_NAME);
            } catch (IOException e) {
