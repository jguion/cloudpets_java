...
class OfflineQueryLogic {
            ...
            if (object.isDataAvailable()) {
                ...
                switch (key.hashCode()) {
                    ...
                    case -1836974455:
                        ...
                        break;
                    case 90495162:
                        if (key.equals(Profile.OBJECT_ID)) {
                            i = 0;
                            break;
                        }
                        break;
    ...
    private <T extends ParseObject> ConstraintMatcher<T> createSelectMatcher(ParseUser user, Object constraint, String key) {
        ...
        return new SubQueryMatcher<T>(user, ((Builder) constraintMap.get(SearchIntents.EXTRA_QUERY)).build()) {
            protected boolean matches(T object, List<T> results) throws ParseException {
    ...
    private <T extends ParseObject> ConstraintMatcher<T> createOrMatcher(ParseUser user, ArrayList<QueryConstraints> queries) {
        ...
        while (it.hasNext()) {
            matchers.add(createMatcher(user, (QueryConstraints) it.next()));
        }
        return new ConstraintMatcher<T>(user) {
            public Task<Boolean> matchesAsync(final T object, final ParseSQLiteDatabase db) {
                ...
                while (it.hasNext()) {
                    final ConstraintMatcher<T> matcher = (ConstraintMatcher) it.next();
            ...
            Collections.sort(results, new Comparator<T>() {
                public int compare(T lhs, T rhs) {
                    ...
                    for (String key : keys) {
                        ...
                        String key2;
                        if (key2.startsWith(Constants.FILENAME_SEQUENCE_SEPARATOR)) {
                            descending = true;
    ...
    private static Task<Void> fetchIncludeAsync(OfflineStore store, Object container, String path, ParseSQLiteDatabase db) throws ParseException {
        if (container == null) {
        ...
        if (container instanceof Collection) {
            ...
            for (final Object item : collection) {
                ...
                task = task.onSuccessTask(new Continuation<Void, Task<Void>>() {
                    public Task<Void> then(Task<Void> task) throws Exception {
                        return OfflineQueryLogic.fetchIncludeAsync(offlineStore, item, str, parseSQLiteDatabase);
                    }
                });
            }
            return task;
            ...
            for (int i = 0; i < array.length(); i++) {
                final int index = i;
                final OfflineStore offlineStore2 = store;
                final String str2 = path;
                final ParseSQLiteDatabase parseSQLiteDatabase2 = db;
                task = task.onSuccessTask(new Continuation<Void, Task<Void>>() {
                    public Task<Void> then(Task<Void> task) throws Exception {
                        return OfflineQueryLogic.fetchIncludeAsync(offlineStore2, array.get(index), str2, parseSQLiteDatabase2);
                    }
                });
            }
            return task;
            ...
            final OfflineStore offlineStore3 = store;
            parseSQLiteDatabase = db;
            offlineStore = store;
            ...
            return Task.forResult(null).continueWithTask(new Continuation<Void, Task<Object>>() {
                public Task<Object> then(Task<Void> task) throws Exception {
                    if (obj instanceof ParseObject) {
                        return OfflineQueryLogic.fetchIncludeAsync(offlineStore3, obj, null, parseSQLiteDatabase).onSuccess(new Continuation<Void, Object>() {
                ...
                public Task<Void> then(Task<Object> task) throws Exception {
                    return OfflineQueryLogic.fetchIncludeAsync(offlineStore, task.getResult(), rest, parseSQLiteDatabase3);
                }
            });
    ...
    static <T extends ParseObject> Task<Void> fetchIncludesAsync(final OfflineStore store, final T object, State<T> state, final ParseSQLiteDatabase db) {
